...
public class ColorDetecPipeline extends OpenCvPipeline {
    enum TeamElementLoc {
        LEFT,
        RIGHT,
        NONE
    ...
    TeamElementLoc location;

    /**
     *
     * @param width The width of the image (check your camera)
     */
    public ColorDetecPipeline(int width) {
        this.width = width;
    }
    ...
    public Mat processFrame(Mat input) {
        ...
        Scalar lowHSV = new Scalar(220, 100, 100); // lower bound HSV for purple
        Scalar highHSV = new Scalar(280, 255, 255); // higher bound HSV for purple
        Mat thresh = new Mat();
        ...
        if (!left) location = TeamElementLoc.LEFT;
        else if (!right) location = TeamElementLoc.RIGHT;
        ...
        else location = TeamElementLoc.NONE;

        return mat; // return the mat with rectangles drawn
    }

    public TeamElementLoc getLocation() {
